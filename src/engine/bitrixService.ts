...
import { reconcileGst } from '../utils/financeUtils';
...
export const BITRIX_SOLUTION_ENUM_MAP: Record<string, string> = {
  '1188': 'CCTV Solution',
...
export const BITRIX_LOST_REASON_MAP: Record<string, string> = {
...
// Bitrix Deal Closure Probability (UF_CRM_1745298149375)
export const BITRIX_CLOSURE_PROBABILITY_MAP: Record<string, { value: number; label: string }> = {
...
export function parseBitrixClosureProbability(val: any): { value: number | null; label: string | null } {
  ...
  try {
    ...
    if (raw) {
      ...
      const sanitizeRecords = (records: DealRecord[]) => (records || []).map(r => {
        ...
        const lostReasonEnum = String(r.rawRecord?.UF_CRM_1786343383165 || '').trim();
        const lostReasonText = r.rawRecord?.UF_CRM_1742536927863 || r.rawRecord?.UF_CRM_67EBCBB2F3CE7 || r.lostReason || '';
...
const parseTitleParts = (title: string, rawRecord?: any) => {
...
export const fetchBitrixLeads = async (customConfig?: BitrixConfig): Promise<BitrixLeadRecord[]> => {
  ...
  const baseUrl = config.webhookBaseUrl ? (config.webhookBaseUrl.endsWith('/') ? config.webhookBaseUrl : `${config.webhookBaseUrl}/`) : '';

  if (!baseUrl || !baseUrl.startsWith('http')) {
    throw new Error('Direct browser-to-Bitrix calls are disabled for security. Bitrix24 CRM integration is managed exclusively server-side. Please configure BITRIX_WEBHOOK_URL in your backend server .env file.');
  }
